...
Re-used ReLU's are renamed because of an error in Captum:
...
__all__ = ['ResNet', 'resnet50', 'resnet50_adapted', 'load_state_dict']
...
class resnet50_adapted(torch.nn.Module):
    def __init__(self, weight_file, num_classes):
        super().__init__()
        
        # load VGG-Face-2 pretrained weights
        pretrained_model = resnet50(num_classes=8631, include_top=True)
        load_state_dict(pretrained_model, weight_file)
